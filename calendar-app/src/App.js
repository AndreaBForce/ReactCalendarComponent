...
function App() {
  ...
  const local_calendar = ["Home","Work","Sport"]
  ...
  const getData=(url,setX)=>{
    ...
    //Legge i dati delle date e li salva nel const data
    useEffect(()=>{
    getData('https://supsi-events.herokuapp.com/bff/events',setData)
    },[])

    //Legge i dati delle date e li salva nel const data
    useEffect(()=>{
    getData('https://supsi-events.herokuapp.com/bff/calendars',setCalendars);
    },[])
